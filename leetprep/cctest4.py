...
def q1(nums):
    ...
    for i in range(len(nums) - 1, -1, -1):
        ...
        prefix = prefix * result[i] #multiply rightmost 
...
def q3(temps):
    ...
    for right in range(len(temps)):
        while temps[right] > stack[-1]:
            idx = stack.pop()
            result[idx] = right - idx # for that weather, today minus that weather
        stack.append(temps [right])
    return result 
...
def q8(target, nums):
    ...
    for right in range(len(nums)):
        ...
        while window_sum > target:
            minlen = min(minlen, right - left + 1)  #greater or equal 
...
def q9(nums, k):
    ...
    for num, freq in count.items():
        buckets[freq].append(num) #higher frequency means higher number
